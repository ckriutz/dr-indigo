...
Respond with ONLY "EMERGENCY" or "NOT_EMERGENCY" when using the chat agent, and with ONLY true/false when used in an executor that expects a boolean.
...
class IsMedicalEmergencyResult(BaseModel):
...
def create_executor_agent(client: AzureOpenAIChatClient) -> AgentExecutor:
    agent = AgentExecutor(
        client.create_agent(
            ...
            response_format=IsMedicalEmergencyResult,
        ),
        ...
        id="medical_emergency_triage_agent_executor",
    )
    return agent
...
def create_agent(client: AzureOpenAIChatClient):
    agent = ChatAgent(
        chat_client=client,
        name="MedicalEmergencyTriageAgent",
        instructions=MEDICAL_EMERGENCY_INSTRUCTIONS,
    )
