...
def agent_task(*, item, endpoint: str, **kwargs) -> Dict[str, any]:
    ...
    try:
        ...
        return {"response": f"Error: {str(e)}", "response_time": response_time}
...
"""
    ...
    try:
        ...
        return Evaluation(
            name="accuracy", value=0.0, comment=f"Evaluation error: {str(e)}"
        )
...
def response_time_evaluator(
    *, input, output, expected_output, metadata, **kwargs
    ...
    """
    response_time = (
        output.get("response_time", 0.0) if isinstance(output, dict) else 0.0
    )
...
def average_accuracy_evaluator(*, item_results, **kwargs) -> Evaluation:
    ...
    if not accuracies:
        return Evaluation(
            name="avg_accuracy", value=None, comment="No accuracy scores found"
        )
...
def run_experiment(
    ...
    result = dataset.run_experiment(
        ...
        or f"Testing Dr. Indigo agent with endpoint {endpoint}",
        ...
        metadata={"endpoint": endpoint, "server_url": SERVER_URL, "agent": "dr-indigo"},
    )
