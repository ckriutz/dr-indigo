...
def compare_with_llm(
    input_text: str, response: str, expected_output: str, llm_client: AzureOpenAI
) -> Tuple[int, str]:
    """
...
"""
    ...
    try:
        completion = llm_client.chat.completions.create(
            model=AUBREY_SETTINGS.azure_openai_triage_model,
...
def process_single_item(
    item: Dict[str, str], item_number: int, llm_client: AzureOpenAI, endpoint: str
) -> Dict:
    """
    ...
    score, explanation = compare_with_llm(
        input_text, response, expected_output, llm_client
    )
...
def run_evaluation(
    ...
    llm_client = AzureOpenAI(
        ...
        azure_endpoint=AUBREY_SETTINGS.azure_openai_endpoint,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = {
            executor.submit(
                process_single_item, item, idx + 1, llm_client, endpoint
            ): idx
...
def main():
    ...
    if output_path is None:
        ...
        output_path = os.path.join(
            os.path.dirname(__file__), f"evaluation_results_{timestamp}.json"
        )

    # Run evaluation
    try:
        evaluation_results = run_evaluation(
            csv_path, output_path, args.n_items, args.max_workers, args.endpoint
        )
